...
class WebsiteSupportTicket(models.Model):
    ...
    person_name = fields.Char(string='Equipment user')
    ...
    problem = fields.Many2many('problem_many2many_tags', string="Problem")
    problem2 = fields.Many2one('problem_many2many_tags', string="Problem2")
    ...
    #convert_time_to_close = fields.Float(string="Time to close (float_time)" )
    ...
    item = fields.Many2one('website.supportzayd.ticket.item', string='Equipment (Serial Number)')
    ...
    def _onchange_partner_id(self):
        if self.partner_id:
            if self.partner_id.zip or self.partner_id.country_id.name or self.partner_id.street2 or self.partner_id.city or self.partner_id.state_id.name:
                address = "{street2}, {city}, {state}, {zip}, {country}".format(
    ...
    def _onchange_item(self):
        ...
        if self.item:
            ...
            if self.item.customer:
                ...
                self.person_name = self.item.user
            else:
